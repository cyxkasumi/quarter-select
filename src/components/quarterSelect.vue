...
<template>
  <div class="v-quarter-select">
    ...
    <input class="quarter-select-input" v-model="quarterFormat" :placeholder="placeholder" @click.stop="selectShowFun" />
    <div class="select-picker" v-show="selectShow" @click.stop>
      ...
      <div class="select-quarter" v-show="!yearShow">
        <p :class="['quarter-item', currentYear == year &&  quarterNum == item ? 'current-quarter': '']" @click="selectedQuarterFun(item)" v-for="item in 4">第&nbsp;&nbsp;{{item}}&nbsp;&nbsp;季度</p>
      ...
      <div class="select-quarter" v-show="yearShow">
        <p :class="['quarter-item', currentYear == item ? 'current-quarter': '']" v-for="item in yearRangeList" @click="selectedYearFun(item)">{{item}}</p>
      </div>
    </div>
  </div>
</template>
<script>
  export default {
    ...
    mounted(){
      window.addEventListener("click", this.selectHidenFun);
    },
    methods: {
      selectShowFun(e){
      ...
      selectedQuarterFun(item){
        ...
        this.$emit("update", this.currentYear + '-' + item);
        this.$emit("change", this.currentYear + '-' + item);
        this.selectShow = false
      },
      // 选择年份 展示年份范围
      yearSelectFun(){
      ...
      yearRangePreFun(){
        ...
        this.yearRange = (parseInt(yearArr[0]) - 12).toString() + '-' + (parseInt(yearArr[1]) - 12).toString()
        this.yearRangeList = []
        for (let i = 0; i < 12; i ++){
